...
\begin{myproof}[Proof.]
	...
	{\color{blue}
	\begin{align*}
	V^K_\gamma(y, \lambda) & = \E_y\left[X_{i,1}\right] + \frac{\gamma}{1-\gamma} \E_y\left[\max\left(\lambda, V^{K-1}_\gamma(y_{i,1}, \lambda)\right) \right],
	\end{align*}
	}
...
\end{itemize}
Recall that the Gittins index policy is optimal for a geometrically distributed horizon with mean $T$. Since FOGI$(T)$ is precisely an approximation for that policy, we would expect it to perform well in our experiments when the horizon is $T$ (even though really it ought to be gemoetrically distributed).

We reuse the two main experimental setups from Section~\ref{sec:experiments}: the Gaussian bandit with 10 independent arms, and the Bernoulli equivalent. Notice from Table~\ref{table:gaussian_experiment2}, in the Guassian setup, that there is value in knowing the true horizon $T$ because FOGI($T$) is the dominant policy. 
...
{\color{blue}
\subsection{Additional benchmark algorithms}
In this section, we simulate a few additional algorithms to understand the importance of the varying discount factor, and to try out a different approximation of the Gittins index. The algorithms we experiment with are:
\begin{itemize}
	\item OGI with a one-step lookahead and a fixed discount factor of $\gamma$, which we will refer to throughout as ``FOGI($1/(1-\gamma)$)". The quantity $1/(1-\gamma)$ can be interpreted as a rough horizon over which this policy is optimal.
	\item OGI in which the Gittins index approximation equals the closed-form expression given in \cite{brezzi2002optimal}. We will refer to this policy as ``BL-OGI".
	\item The greedy policy, which plays the arm in $\argmax_i \E_{y_{i, N_i(t-1)}}[X_{i,t}]$. Effectively it is equivalent to FOGI($1$), and completely disregards the value of future exploration. We will simply call this policy ``Greedy" in our tables and plots.
\end{itemize}
We reuse the two main experimental setups from Section~\ref{sec:experiments}

Wee see from Table~\ref{table:gaussian_experiment2} that there is value in knowing the true horizon $T$ since FOGI($T$) approximates the optimal Gittins index policy and so performs well. It's also worth highlighting that because the Gittins index policy is near-optimal for this particular choice of horizon, our approximation is effective during the running of the algorithm.
...
{\color{blue} The comparison against FOGI, BL-OGI and Greedy in the Bernoulli case, presented in Table~\ref{table:bernoulli_experiment2}, tells a similar story as in Table~\ref{table:gaussian_experiment2}.
}
