...
\subsection{Analysis of $v^K_\gamma(y)$}
...
\begin{myproof}[Proof.]
	Let $K \le M$ be two lookahead parameters used in the definition of OGI. We will show that $V^K_\gamma(y, \lambda) \le V^M_\gamma(y, \lambda)$ where we recall the definitions of these functions from the beginning of Section~\ref{sec:appendix_properties_of_ogi}.
	
	We begin with a fundamental step. Let $\tau_1$ and $\tau_2$ be any stopping times such that $\tau_1$ precedes $\tau_2$ almost surely, that is $\tau_1 \le \tau_2$. Recall that the expected reward of the $i$th arm satisfies $\Ee{X_{i,t} \given \theta_i} = \mu(\theta_i)$ for all $t$. Let $\hat \theta_i \in \Theta$ denote a realization of the random variable $\theta_i$ and let $\zeta(\hat \theta_i)$ be a real-valued, measurable function of $\hat \theta_i$. In this case, we have that
	\begin{align*}
	\Ee{\sum_{t=\tau_1}^{\tau_2-1} \gamma^{t-1} X_{i,t} + \gamma^{\tau_2-1}\frac{\zeta(\hat \theta_i)}{1 - \gamma} \given[\Bigg] \theta_i = \hat \theta_i} & =\mu(\hat \theta_i) \Ee{ \sum_{t=\tau_1}^{\tau_2-1} \gamma^{t-1}  \given[\Bigg] \theta_i = \hat \theta_i} + \Ee{\frac{\gamma^{\tau_2-1}}{1-\gamma}\given[\Bigg] \theta_i = \hat \theta_i}\zeta(\hat \theta_i) \\
	& =\mu(\hat \theta_i) \Ee{ \sum_{t=\tau_1}^{\tau_2-1} \gamma^{t-1} \given[\Bigg] \theta_i = \hat \theta_i } + \Ee{\sum_{t=\tau_2}^{\infty} \gamma^{t-1} \given[\Bigg] \theta_i = \hat \theta_i}\frac{\zeta(\hat \theta_i)}{1 - \gamma} \\
	& \le \Ee{\gamma^{\tau_1-1} \given \theta_i = \hat \theta_i}  \frac{\max(\zeta(\hat \theta_i), \mu(\hat \theta_i))}{1-\gamma}
	\end{align*}
	...
	\Ee{\sum_{t=\tau_1}^{\tau_2-1} \gamma^{t-1} X_{i,t} + \gamma^{\tau_2-1}\frac{\zeta(\hat \theta_i)}{1 - \gamma} \given[\Bigg] \theta_i}  \le \Ee{\gamma^{\tau_1-1} \given \theta_i }  \frac{\max(\zeta(\hat \theta_i), \mu(\theta_i))}{1-\gamma}.
	\end{equation}
	Let $\tau^\star$ be a stopping time that achieves the supremum in  $V_\gamma^M(y, \lambda)$ and define the stopping time $\tau^\star_K \defeq (K+1) \wedge \tau^\star$. Consider the (conditional) cumulative rewards in the definition of $V^M_\gamma(y)$, from time $\tau^\star_K$ onwards, given the sufficient statistic observed at time $\tau_K^\star - 1$. That is, $\E\left[\sum_{t=\tau_K^\star}^{\tau^\star}  \gamma^{t-1} X_{i,t} + \gamma^{\tau^\star-1} R_{\lambda,M}(\tau^\star, y_{i,\tau^\star-1})/(1-\gamma)
	\given[\Bigg] y_{i,\tau_K^\star-1} \right]$. We upper bound this random variable as follows. First, we note that, at any time $s$ and for any statistic $\hat y \in \mathcal{Y}$, the following statement holds
	...
	\begin{align*}
		& \gamma^{\tau^\star-1} \frac{R_{\lambda,M}(\tau^\star, y_{i,\tau^\star-1})}{1-\gamma}
		 \nonumber \\
		&\quad =  \gamma^{\tau^\star-1}\left( \ind{\tau^\star =M+1}\frac{\max(\lambda, R(y_{i,\tau^\star-1}))}{1-\gamma} + \ind{\tau^\star < M+1}\frac{\lambda}{1-\gamma}\right)
		 \nonumber\\
		&\quad= \ind{\tau^\star =M+1} \gamma^{M}\frac{\max(\lambda, R(y_{i,M}))}{1-\gamma} + \ind{\tau^\star < M+1} \gamma^{\tau^\star-1}\frac{\lambda}{1-\gamma}
	\nonumber\\
		& \quad\overset{(*)}{=} \ind{\tau^\star =M+1} \gamma^{M}\frac{\Ee{\max(\lambda, R(y_{i,M})) \given y_{i,M} }}{1-\gamma} + \ind{\tau^\star < M+1} \gamma^{\tau^\star-1}\frac{\lambda}{1-\gamma}
		\\
		& \quad\overset{(\dagger)}{=} \ind{\tau^\star =M+1} \gamma^{M}\frac{\Ee{\max(\lambda, \mu(\theta_i)) \given y_{i,M} }}{1-\gamma} + \ind{\tau^\star < M+1} \gamma^{\tau^\star-1}\frac{\lambda}{1-\gamma}
		 \\
		&\quad \overset{(**)}{\le} \frac{  \Ee{\gamma^{\tau^\star-1}\max(\lambda, \mu(\theta_i)) \given y_{i,\tau^\star-1} }}{1-\gamma}
	\end{align*}
	where $(*)$ and $(**)$ both use the fact that for any $t$, $\tau^\star \ge t$ is measurable with respect to the $\sigma$-algebra generated by $y_{i,t-1}$, namely $\mathcal F_{t}$ (i.e. we can decide whether to stop at any time $t$ given the statistic $y_{i,t-1}$ about the arm). Equation ($\dagger$) follows from \eqref{eqn:dist_equal_theta_i}. Therefore, immediately using the above inequality and conditioning on the event $\tau^\star \ge (K+1)$, we have that
	\begin{align} 
